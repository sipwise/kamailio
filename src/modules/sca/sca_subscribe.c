...
int sca_subscription_save_unsafe(sca_mod *, sca_subscription *, int, int);
...
int sca_subscription_from_db_row_values(db_val_t *values, sca_subscription *sub)
{
	assert(values != NULL);
	assert(sub != NULL);
...
int sca_subscription_to_db_row_values(sca_subscription *sub, db_val_t *values)
{
		...
		sca_subscription *sub)
{
	...
	SCA_DB_BIND_INT_VALUE(sub->dialog.notify_cseq + 1,
			&SCA_DB_NOTIFY_CSEQ_COL_NAME, update_columns, update_values,
			update_column_idx);
...
int sca_subscription_aor_has_subscribers(int event, str *aor)
{
	sca_hash_slot *slot;
	sca_hash_entry *e;
	sca_subscription *sub;
...
sca_subscription *sca_subscription_create(str *aor, int event, str *subscriber,
...
static int sca_subscription_update_unsafe(sca_mod *scam,
		sca_subscription *saved_sub, sca_subscription *update_sub, int sub_idx)
{
	int rc = -1;
	int len;
	char *dlg_id_tmp;
	...
	!STR_EQ(saved_sub->dialog.to_tag, update_sub->dialog.to_tag)) {
		...
		if (dlg_id_tmp == NULL) {
			...
			SCA_STR_APPEND(&saved_sub->dialog.id, &update_sub->dialog.from_tag);
 ...
 */
int sca_subscription_terminate(sca_mod *scam, str *aor, int event,
